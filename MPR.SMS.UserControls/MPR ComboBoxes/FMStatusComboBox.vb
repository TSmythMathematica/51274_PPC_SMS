...
''' </summary>
''' <remarks>
''' Using the <see cref="T:MPR.Windows.Forms.ucComboBoxAutoComplete" />, the
...
#Region "Public Properties"
    ...
    Public Property SelectedStatus() As FMStatus
        Get
            ...
            If cboStatuses.SelectedIndex = -1 Then
                Return Nothing
            Else
                Return CType(cboStatuses.Items(cboStatuses.SelectedIndex), FMStatus)
            End If

        End Get

        Set(ByVal Value As FMStatus)
...
#Region "Event Handlers"
    ...
    Private Sub cboStatuses_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboStatuses.SelectedIndexChanged

        RaiseEvent SelectedIndexChanged(Me, e)

    End Sub

    Private Sub cboStatuses_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cboStatuses.KeyUp
...
#Region "Private Methods"
    ...
    Private Sub FillList()
        ...
        Dim strSQL As String = "SELECT * FROM tlkpFMStatus "
        If mFilters.Length > 0 AndAlso Not mFilters.Substring(0, 5).ToUpper.Equals("WHERE") Then
            strSQL += "WHERE "
        ElseIf mFilters.Length = 0 Then
            strSQL += "WHERE IsActive = 1"  'limit to active statuses only. Other callers who pass a filter should do this as well.
        End If
        strSQL += mFilters
        strSQL += " order by CONVERT(INT, code) "
        ...
        If ds IsNot Nothing AndAlso ds.Tables.Count > 0 Then
            ds.Tables(0).DefaultView.RowFilter = "IsActive = 1"  'dbl-check incase the caller didn't do this in their SQL filter.
            For Each dr As DataRow In ds.Tables(0).DefaultView.Table.Rows
                objStatus = New FMStatus(dr)
