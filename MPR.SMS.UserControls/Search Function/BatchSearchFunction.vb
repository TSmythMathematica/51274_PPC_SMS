...
Public Class BatchSearchFunction
...
#Region "Public Properties"
    ...
    Public Property PrintBatchSheetButtonVisible As Boolean
        Get
            Return Me.btnPrintBatchSheet.Visible
        End Get
        Set
            Me.btnPrintBatchSheet.Visible = Value
        End Set
    End Property
...
#Region "Event Handlers"
    ...
    Private Sub dgvSearchResults_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) _
        ...
        '    RaiseEvent SearchResultDoubleClick(sender, e, CType(dgvSearchResults.SelectedRows(0).DataBoundItem, DataRowView).Row)
        'End If
    End Sub
    ...
    Private Sub btnPrintBatchSheet_Click(sender As Object, e As EventArgs) Handles btnPrintBatchSheet.Click
        If dgvSearchResults.SelectedRows(0) IsNot Nothing Then
            Try
                Dim intBatchId As Integer
                Dim strbatchid As String

                strbatchid = Trim(dgvSearchResults.SelectedRows(0).Cells("intBatchID").Value.ToString)
                intBatchId = CInt(strbatchid)

                'SSRS Version
                Dim parameter As New ParameterValue
                With parameter
                    .Name = "BatchID"
                    .Value = GetSafeValue(intBatchId).ToString
                End With
